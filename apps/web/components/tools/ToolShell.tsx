...
export function ToolShell({
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col px-5 py-10 sm:py-14">
      ...
      <Link href="/tools" className="mt-8 inline-flex items-center gap-1 text-sm text-white/50 hover:text-white/80">
        ← All tools
      </Link>

      <div className="mt-3 flex items-start gap-3.5">
        <span
          className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-2xl"
          aria-hidden
        >
          {icon}
        </span>
        <div>
          ...
          <p className="mt-1.5 max-w-2xl text-white/60">{blurb}</p>
        </div>
      </div>
      ...
      {server ? (
        <p className="mt-10 flex items-center gap-2 rounded-xl border border-sky-500/20 bg-sky-500/5 px-4 py-3 text-xs text-sky-200/80">
      ...
      {others.length > 0 && (
        <nav className="mt-8">
          <h2 className="text-xs uppercase tracking-wide text-white/40">More tools</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {others.map((t) => (
              <Link
                key={t.slug}
                href={`/tools/${t.slug}`}
                className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white/70 transition hover:border-primary/40 hover:text-white"
              >
