...
const SEVERITY_META: Record<Severity, { label: string; cls: string; dot: string }> = {
  critical: { label: 'Critical', cls: 'border-red-500/30 bg-red-500/10 text-red-600', dot: 'bg-red-400' },
  high: { label: 'High', cls: 'border-orange-500/30 bg-orange-500/10 text-orange-600', dot: 'bg-orange-400' },
  medium: { label: 'Medium', cls: 'border-amber-500/30 bg-amber-500/10 text-amber-600', dot: 'bg-amber-400' },
    ...
    <div className="space-y-5">
      ...
      {total > 0 && (
        <>
            ...
            {result.hits.map((hit, i) => {
              ...
              return (
                <div
                  ...
                  className="flex items-start gap-3 rounded-xl border border-ink/10 bg-white p-3"
                >
                  <span
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-ink">{hit.provider}</p>
          ...
          <p className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-xs text-amber-700/80">
